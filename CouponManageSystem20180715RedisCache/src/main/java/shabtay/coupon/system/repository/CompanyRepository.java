...
public interface CompanyRepository extends CrudRepository<Company, Long> {
	...
	// This Query : Joins between Company entity and Coupons entity (using  comp.coupons AS cp ) ,
	// then search for coupons per Company ID And CouponType
	@Query("SELECT cp FROM COMPANY AS comp INNER JOIN comp.coupons AS cp WHERE comp.id=:id AND cp.type=:couponType")
	...
	// This Query : Joins between Company entity and Coupons entity (using  comp.coupons AS cp ) ,
	// then search for coupons per Company ID And between range of prices (minPrice, maxPrice)  
	@Query("SELECT cp FROM COMPANY AS comp INNER JOIN comp.coupons AS cp WHERE comp.id=:id AND cp.price>=:minPrice AND cp.price<=:maxPrice")
	...
	// This Query : Joins between Company entity and Coupons entity (using  comp.coupons AS cp ) ,
	// then search for coupons per Company ID And between dates (startDate, endDate)  
	@Query("SELECT cp FROM COMPANY AS comp INNER JOIN comp.coupons AS cp WHERE comp.id=:id AND cp.startDate>=:startDate AND cp.endDate<=:endDate")
	List<Coupon> findCouponBetweenDates(@Param("id") long compId, @Param("startDate") Date startingDate, @Param("endDate") Date endingDate);

	
	
	@Query("SELECT cp FROM COMPANY AS comp INNER JOIN comp.coupons AS cp WHERE comp.id = :id")
	List<Coupon> getAllCoupons(@Param("id") long compId);
	
	
	
	@Query("SELECT cp FROM COMPANY AS comp INNER JOIN comp.coupons AS cp WHERE comp.id=:id AND cp.title=:title")
	Coupon getCouponByTitle(@Param("id")long compId,@Param("title")String title);
	
	
	@Query("SELECT cp FROM COMPANY AS comp INNER JOIN comp.coupons AS cp WHERE comp.id=:id AND cp.id=:coupId")
	Coupon getCouponById(@Param("id")long compId, @Param("coupId")long coupId);
